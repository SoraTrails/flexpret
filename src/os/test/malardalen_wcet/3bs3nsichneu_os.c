...
int main() {
	...
	osKernelInitialize();
	osThreadId_t bs_th[3];
	osThreadId_t ns_th[3];
	register int i;
	for (i = 0; i < 3; i++) {
		bs_th[i] = osThreadNew(bs_main, NULL, &attr[i]);
	}
	for (i = 0; i < 3; i++) {
		ns_th[i] = osThreadNew(nsichneu_main, NULL, &attr[i + 3]);
	}
	osKernelStart();
	for (i = 0; i < 3; i++) {
		osThreadJoin(bs_th[i]);
	}
	for (i = 0; i < 3; i++) {
		osThreadJoin(ns_th[i]);
	}
	return 0;
}