...
int test_trap() {
    ...
    osKernelStart();
    FLEXPRET_ASSERT(osKernelSetTrapHandler(tid, (trap_handler)fault_handler, Fault) == osOK);
    ...
    FLEXPRET_ASSERT(osKernelGetTrapHandler(tid, Fault) == (trap_handler)fault_handler);
